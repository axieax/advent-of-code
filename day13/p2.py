...
# with open('input.txt', 'r') as f:
    ...
    # # list of tuples: bus_id (ignoring 'x'), bus_index (add to t)
    # restricted_bus_services = [(int(bus_id), index) for (index, bus_id) in enumerate(all_bus_services) if bus_id != 'x']
    # highest_bus_id, highest_bus_index = max(restricted_bus_services)
    # t = first_multiple_above_earliest(highest_bus_id, 100000000000000) - highest_bus_index
    # while True:
    #     if all(map(lambda bus_tuple: (t + bus_tuple[1]) % bus_tuple[0] == 0, restricted_bus_services)):
    #         print(t)
    #         break
    #     t += highest_bus_id
